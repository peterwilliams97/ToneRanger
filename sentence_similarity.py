...
if do_got:
    ...
    for book_filename in book_filenames:
        print("Reading '{0}'... ".format(book_filename), end='')
...
model_name = '%s.%d.w2v' % (model_name, min_word_count)
...
if do_tokenize:
    ...
    sentences = []
    print('%7d sentences' % (len(raw_sentences)))
    ...
    for i, raw in enumerate(raw_sentences):
        ...
        if i % interval == 0 and interval:
            print('%7d (%5.1f%%) %6.1f sec' % (i, 100.0 * i / len(raw_sentences), time.clock() - t0))

    print('raw sentence 5:', raw_sentences[5])
    print('word list 5:', sentence_to_wordlist(raw_sentences[5]))
...
if do_train:
    ...
    print(type(thrones2vec.wv.vocab))
...
def show_similer(term):
    ...
    print('Most similar to %r' % term)
    for i, (t, v) in enumerate(similar):
        print('%4d: %15s %.3f' % (i, t, v))
...
nearest_similarity_cosmul("print", "scan", "Melbourne")
