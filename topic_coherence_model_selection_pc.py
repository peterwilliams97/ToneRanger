...
our_models_dir = os.path.join('pc', 'models')  # use whatever directory you prefer
...
class NewsgroupCorpus(TextCorpus):
    ...
    def __init__(self, *args, **kwargs):
        super(NewsgroupCorpus, self).__init__(*args, **kwargs)
        self.raw_sentences = my_utils.load_json(sentences_path)

    def getstream(self):
        t0 = time.perf_counter()
        for i, doc in enumerate(self.raw_sentences):
            if len(doc) > 0:
                yield doc
            if i % interval == 0 and interval:
                print('%7d (%5.1f%%) %6.1f sec' % (i, 100.0 * i / len(self.raw_sentences),
                    time.perf_counter() - t0))

    def preprocess_text(self, text):
        # body = extract_body(text)
        return super(NewsgroupCorpus, self).preprocess_text(text)
...
if do_2:
    ...
    for num_topics in range(20, 51, 10):
        ...
        t0 = time.perf_counter()
        lda = models.LdaMulticore(mm_corpus, id2word=dictionary, num_topics=num_topics,
                workers=n_workers,
